...
from myutil.disk import getCache
from myutil.util import XyzTuple, xyz2irc
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob('/content/drive/MyDrive/subset/**/*.mhd', recursive=True)
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open('/content/drive/MyDrive/annotations_with_malignancy.csv', "r") as f:
    ...
    with open('/content/drive/MyDrive/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            '/content/drive/MyDrive/subset/**/{}.mhd'.format(series_uid),  recursive=True
        )[0]
