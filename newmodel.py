...
def conv3d(in_channels, out_channels, kernel_size, bias, padding='same'):
    return nn.Conv3d(in_channels, out_channels, kernel_size, padding=padding, bias=bias)


def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding='same'):
    """
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='crg', num_groups=8, padding='same'):
...
class SegmentationAugmentation(nn.Module):
    ...
    def forward(self, input_g, label_g):
        ...
        affine_t = F.affine_grid_3d(transform_t[:,:3],
                input_g.size(), align_corners=False)

        augmented_input_g = F.grid_sample_3d(input_g,
                affine_t, padding_mode='border',
                align_corners=False)
        augmented_label_g = F.grid_sample_3d(label_g.to(torch.float32),
                affine_t, padding_mode='border',
                align_corners=False)
...
class UNetWrapper(nn.Module):
    ...
    def forward(self, input_batch):
        ...
        un_output = self.unet(input_batch)
